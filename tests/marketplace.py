...
def get_order(order_id):
    ...
    GET /orders/{orderId}
    """
    print_request('GET', f"{MARKETPLACE_SERVICE_URL}/orders/{order_id}")
    response = requests.get(f"{MARKETPLACE_SERVICE_URL}/orders/{order_id}")
    print_response(response)
    return response


def test_get_order(order_id, response, expected_status=None):
    """
    Checks GET /orders/{orderId} response
    - Must have 200 if order found
    - Must have 'order_id' (int) == order_id
    - Optionally check 'status' (string) == expected_status
    """
    if not check_json_exists(response):
        return False
    ...
    if not check_field_type(payload, 'order_id', int):
        return False
    if not check_field_value(payload, 'order_id', order_id):
        return False
    ...
    if expected_status is not None:
        ...
        if payload['status'] != expected_status:
            print_fail_message(f"Order {order_id} status mismatch. Expected {expected_status}, got {payload['status']}")
            return False
        else:
            print_pass_message(f"Order {order_id} status is correct => {expected_status}")

    # We expect 200 if the order exists
    if not check_response_status_code(response, 200):
        return False
